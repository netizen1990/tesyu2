...
function showBanner() {
    ...
    console.log(chalk.hex('#00ff00').bold('║                                                 403 Tools v2.0                                              ║'));
    console.log(chalk.hex('#00ff00').bold('║                                              [AUTHOR: 403 Bypass]                                                   ║'));
    console.log(chalk.hex('#00ff00').bold('╚════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝'));
    console.log('');
}
...
// Fungsi untuk input manual target URL dengan animasi
    ...
    return new Promise((resolve) => {
        rl.question(chalk.hex('#00ff00')('[TARGET] Masukkan URL: '), async (url) => {
            try {
                ...
                spinner.succeed(chalk.green('Target valid: ') + chalk.white(url));
                resolve();
            } catch (error) {
                log('error', 'URL tidak valid! Masukkan URL yang benar');
                getTargetUrl().then(resolve);
            }
        });
    });
}
        ...
        rl.question(chalk.hex('#00ff00')('[MODE] Pilih (1-3): '), (answer) => {
            switch(answer) {
                ...
                case '3':
                    ...
                    break;
                default:
                    log('warn', 'Mode tidak valid, menggunakan BALANCED');
                    CONFIG.mode = 'balanced';
            }
...
// Fungsi untuk auto-detect proteksi dengan tampilan hacker
async function detectProtection() {
    ...
    try {
        ...
        spinner.fail(chalk.red('Gagal mendeteksi proteksi'));
    }
}

// Fungsi untuk monitoring resource dengan tampilan hacker
...
// Fungsi untuk mendapatkan proxy dengan tampilan hacker
async function fetchProxies() {
    ...
    try {
        ...
        spinner.fail(chalk.red('Gagal mengambil proxy'));
        return false;
    } catch (error) {
...
// Fungsi untuk menyelesaikan CAPTCHA dengan tampilan hacker
...
// Fungsi untuk menjalankan instance browser dengan tampilan hacker
