...
const DEFAULT_CONFIG = {
    maxConcurrent: 2, // Dikurangi untuk stabilitas maksimal
    requestsPerInstance: 10, // Dikurangi untuk menghindari resource overload
    proxyRefreshInterval: 3,
    timeout: 90000, // Diperpanjang untuk stabilitas
    outputDir: './results',
    ...
    minDelay: 3000, // Diperpanjang untuk menghindari deteksi
    maxDelay: 7000,
    scrollBehavior: true,
    mouseMovement: true,
    ...
    maxCpuUsage: 70, // Dikurangi untuk safety margin
    maxRamUsage: 70, // Dikurangi untuk safety margin
    logLevel: 'info',
    ...
    browserArgs: [
        ...
        '--use-mock-keychain',
        '--single-process', // Untuk stabilitas di resource terbatas
        '--disable-features=VizDisplayCompositor' // Memperbaiki masalah di beberapa sistem
    ],
    // Retry configuration yang ditingkatkan
    maxBrowserRetries: 5,
    browserRetryDelay: 3000,
    // Health check configuration
    healthCheckInterval: 30000,
    maxBrowserInstances: 10 // Maksimal browser instances yang diizinkan
};
...
async function launchBrowserWithRetry(proxy, instanceId) {
    ...
    // Clean up any zombie processes before launch
    cleanZombieProcesses();
    
    while (retryCount < CONFIG.maxBrowserRetries) {
        try {
            log('debug', `Instance ${instanceId}: Launching browser (attempt ${retryCount + 1}/${CONFIG.maxBrowserRetries})`);
            ...
            // Get Chromium path if not already set
            if (!chromiumPath) {
            ...
            const browser = await puppeteer.launch({
                ...
                userDataDir: `./temp_${instanceId}`, // Unique user data dir for each instance
            });
            
            // Add to active browsers set
...
async function humanMouseMovement(page) {
    ...
    const moves = 3 + Math.floor(Math.random() * 5); // Dikurangi untuk efisiensi
    ...
    for (let i = 0; i < moves; i++) {
        ...
        await page.mouse.move(x, y, {
            steps: 5 + Math.floor(Math.random() * 10) // Dikurangi untuk efisiensi
        });
...
async function humanScroll(page) {
    ...
    const scrollCount = 2 + Math.floor(Math.random() * 3); // Dikurangi untuk efisiensi
...
async function humanType(page, selector, text) {
    ...
    for (const char of text) {
        await page.keyboard.type(char, {
            delay: 50 + Math.random() * 100 // Dikurangi untuk efisiensi
        });
...
async function runBrowserInstance(instanceId) {
    ...
    try {
        ...
        // Launch browser with enhanced retry mechanism
        browser = await launchBrowserWithRetry(proxy, instanceId);

        const userAgent = randomUseragent.getRandom();
        const viewport = {
            width: 1024 + Math.floor(Math.random() * 200), // Dikurangi untuk stabilitas
            height: 768 + Math.floor(Math.random() * 200), // Dikurangi untuk stabilitas
            deviceScaleFactor: 1,
            isMobile: false,
            hasTouch: false
        };
...
async function main() {
    ...
    for (let i = 1; i <= CONFIG.maxConcurrent; i++) {
        ...
        await new Promise(resolve => setTimeout(resolve, 5000)); // Increased delay between instances
    }
