...
const DEFAULT_CONFIG = {
    ...
    timeout: 45000,
    outputDir: './results',
    proxyApiUrl: 'http://pubproxy.com/api/proxy?format=json&limit=10&level=anonymous&speed=5&country=us&type=http',
...
function isValidUrl(urlString) {
    try {
        ...
        // Validasi TLD (minimal 2 karakter setelah titik terakhir)
...
// Fungsi untuk input manual target URL dengan validasi yang diperbaiki
        ...
        const askForUrl = () => {
            rl.question(chalk.hex('#00ff00')('[TARGET] Masukkan URL: '), async (url) => {
                try {
                    ...
                    const spinner = ora({
                        text: 'Memvalidasi target...',
                        spinner: 'dots',
                        color: 'green'
                    }).start();
                    
                    // Coba akses URL untuk memastikan valid
                    try {
                        ...
                        spinner.succeed(chalk.green('Target valid dan dapat diakses: ') + chalk.white(targetUrl));
                        resolve();
                    } catch (accessError) {
                        // Jika gagal diakses, tanyakan apakah ingin melanjutkan
                        spinner.warn(chalk.yellow('Target tidak dapat diakses, tapi URL valid'));
                        rl.question(chalk.hex('#00ff00')('[TARGET] Lanjutkan anyway? (y/n): '), (answer) => {
                            if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
                                spinner.succeed(chalk.green('Melanjutkan dengan target: ') + chalk.white(targetUrl));
                                resolve();
                            } else {
                                askForUrl();
                            }
...
// Fungsi untuk memilih mode dengan tampilan hacker
...
// Fungsi untuk auto-detect proteksi dengan penanganan error yang diperbaiki
async function detectProtection() {
    if (!CONFIG.autoDetectProtection) return;
    
    const spinner = ora({
        text: 'Mendeteksi proteksi target...',
        spinner: 'dots12',
        color: 'cyan'
    }).start();
    ...
    try {
        const response = await axios.get(targetUrl, {
            ...
            validateStatus: function (status) {
                return status < 500; // Accept any status code except server errors
            }
        });
        ...
        if (server.includes('cloudflare') || poweredBy.includes('cloudflare')) {
            ...
            spinner.succeed(chalk.green('Proteksi terdeteksi: ') + chalk.hex('#ff6600')('CLOUDFLARE'));
            ...
            spinner.succeed(chalk.green('Proteksi terdeteksi: ') + chalk.hex('#00ff00')('NGINX'));
        } else if (server.includes('apache')) {
            protectionType = 'apache';
            spinner.succeed(chalk.green('Proteksi terdeteksi: ') + chalk.hex('#0099ff')('APACHE'));
        } else {
            protectionType = 'unknown';
            spinner.warn(chalk.yellow('Proteksi tidak dikenali'));
        }
    } catch (error) {
        spinner.fail(chalk.red('Gagal mendeteksi proteksi: ') + chalk.white(error.message));
        protectionType = 'unknown';
    }
}

// Fungsi untuk monitoring resource dengan penanganan error yang diperbaiki
...
// Fungsi untuk mendapatkan proxy dengan penanganan error yang diperbaiki
async function fetchProxies() {
    const spinner = ora({
        text: 'Mengambil proxy...',
        spinner: 'bouncingBar',
        color: 'magenta'
    }).start();
    ...
    try {
        ...
        if (response.data && response.data.data && Array.isArray(response.data.data)) {
            ...
            if (proxyList.length > 0) {
                spinner.succeed(chalk.green(`Proxy diperoleh: `) + chalk.white(`${proxyList.length} proxy`));
                return true;
            } else {
                spinner.warn(chalk.yellow('Tidak ada proxy baru tersedia'));
                return false;
            }
        } else {
            spinner.fail(chalk.red('Respon proxy tidak valid'));
            return false;
        }
    } catch (error) {
        spinner.fail(chalk.red('Proxy error: ' + error.message));
        return false;
    }
}
...
// Fungsi untuk menyelesaikan CAPTCHA dengan penanganan error yang diperbaiki
async function solveCaptcha(page) {
    ...
    try {
        ...
        if (!siteKey) {
            spinner.fail(chalk.red('Site key tidak ditemukan'));
            return false;
        }
        ...
        captchaSolvedCount++;
        spinner.succeed(chalk.green('CAPTCHA cracked!'));
        return true;
    } catch (error) {
        spinner.fail(chalk.red('CAPTCHA error: ' + error.message));
        return false;
    }
}

// Fungsi untuk menjalankan instance browser dengan penanganan error yang diperbaiki
...
// Fungsi untuk menampilkan hasil akhir dengan gaya hacker
...
async function main() {
    ...
    // Setup progress bar dengan gaya hacker
    ...
    // Tunggu semua instances selesai
    await Promise.all(instances);

    // Stop progress bar dan monitoring
    progressBar.stop();
